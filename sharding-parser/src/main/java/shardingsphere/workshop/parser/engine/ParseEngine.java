...
import shardingsphere.workshop.parser.engine.visitor.*;
...
public final class ParseEngine {
    ...
    public static ASTNode parseUpdate(final String sql){
        ParseTree parseTree = createParseTree(sql);
        return new UpdateSqlVisitor().visit(parseTree.getChild(0));
    }

    public static ASTNode parseSelect(final String sql){
        ParseTree parseTree = createParseTree(sql);
        return new SelectSqlVisitor().visit(parseTree.getChild(0));
    }

    public static ASTNode parseDelete(final String sql){
        ParseTree parseTree = createParseTree(sql);
        return new DeleteSqlVisitor().visit(parseTree.getChild(0));
    }
